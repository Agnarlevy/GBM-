...
alvotech_data = yf.download(
    "ALVO", start="2023-01-01", end="2023-12-31", progress=False)
...
actual_price = alvotech_data['Close'].iloc[-1]  # Last closing price of 2023

s0_points = {
    '1-day': alvotech_data['Close'].iloc[-2],  # 1 day before
    '1-week': alvotech_data['Close'].iloc[-6],  # 5 trading days before
    '1-month': alvotech_data['Close'].iloc[-21]  # 20-22 trading days before
}
...
for horizon, time_interval in time_horizons.items():
    S_0 = s0_points[horizon]  # Adjust S_0 based on the horizon
    drift, sigma_star = calculate_parameters(alvotech_data, time_interval)
...
plt.title("GBM Simulations for Alvotech (ALVO)")
