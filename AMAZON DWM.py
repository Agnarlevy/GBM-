...
amazon_data = yf.download("AMZN", start="2023-01-01",
...
actual_price = amazon_data['Close'].iloc[-1]  # Last closing price of 2023

s0_points = {
    '1-day': amazon_data['Close'].iloc[-2],  # 1 day before
    '1-week': amazon_data['Close'].iloc[-6],  # 5 trading days before
    '1-month': amazon_data['Close'].iloc[-21]  # 20-22 trading days before
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(amazon_data, time_interval)
...
plt.title("GBM Simulations for Amazon (AMZN)")
