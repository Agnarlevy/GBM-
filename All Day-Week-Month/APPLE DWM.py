...
apple_data = yf.download("AAPL", start="2023-01-01",
...
actual_price = apple_data['Close'].iloc[-1]


s0_points = {
    '1-day': apple_data['Close'].iloc[-2],
    '1-week': apple_data['Close'].iloc[-6],
    '1-month': apple_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(apple_data, time_interval)
...
plt.title("GBM Simulations for Apple (AAPL)")
