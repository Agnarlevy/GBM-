...
arion_data = yf.download("ARION.IC", start="2023-01-01",
...
actual_price = arion_data['Close'].iloc[-1]

s0_points = {
    '1-day': arion_data['Close'].iloc[-2],
    '1-week': arion_data['Close'].iloc[-6],
    '1-month': arion_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(arion_data, time_interval)
...
plt.title("GBM Simulations for Arion Banki (ARION.IC)")
