...
berkshire_data = yf.download(
    "BRK-B", start="2023-01-01", end="2023-12-31", progress=False)
...
actual_price = berkshire_data['Close'].iloc[-1]

s0_points = {
    '1-day': berkshire_data['Close'].iloc[-2],
    '1-week': berkshire_data['Close'].iloc[-6],
    '1-month': berkshire_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(berkshire_data, time_interval)
...
plt.title("GBM Simulations for Berkshire Hathaway (BRK-B)")
