...
coloplast_data = yf.download(
    "COLO-B.CO", start="2023-01-01", end="2023-12-31", progress=False)
...
actual_price = coloplast_data['Close'].iloc[-1]


s0_points = {
    '1-day': coloplast_data['Close'].iloc[-2],
    '1-week': coloplast_data['Close'].iloc[-6],
    '1-month': coloplast_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(coloplast_data, time_interval)
...
plt.title("GBM Simulations for Coloplast (COLO-B.CO)")
