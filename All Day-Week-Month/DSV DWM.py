...
dsv_data = yf.download("DSV.VI", start="2023-01-01",
...
actual_price = dsv_data['Close'].iloc[-1]

s0_points = {
    '1-day': dsv_data['Close'].iloc[-2],
    '1-week': dsv_data['Close'].iloc[-6],
    '1-month': dsv_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(dsv_data, time_interval)
...
plt.title("GBM Simulations for DSV (DSV.VI)")
