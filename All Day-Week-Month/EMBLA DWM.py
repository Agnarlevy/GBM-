...
embla_data = yf.download("EMBLA.CO", start="2023-01-01",
...
actual_price = embla_data['Close'].iloc[-1]

s0_points = {
    '1-day': embla_data['Close'].iloc[-2],
    '1-week': embla_data['Close'].iloc[-6],
    '1-month': embla_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(embla_data, time_interval)
...
plt.title("GBM Simulations for Embla (EMBLA.CO)")
