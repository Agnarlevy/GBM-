...
isb_data = yf.download("ISB.IC", start="2023-01-01",
...
actual_price = isb_data['Close'].iloc[-1]

s0_points = {
    '1-day': isb_data['Close'].iloc[-2],
    '1-week': isb_data['Close'].iloc[-6],
    '1-month': isb_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(isb_data, time_interval)
...
plt.title("GBM Simulations for Íslandsbanki (ISB.IC)")
