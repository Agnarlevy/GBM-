...
maersk_data = yf.download(
    "MAERSK-B.CO", start="2023-01-01", end="2023-12-31", progress=False)
...
actual_price = maersk_data['Close'].iloc[-1]


s0_points = {
    '1-day': maersk_data['Close'].iloc[-2],
    '1-week': maersk_data['Close'].iloc[-6],
    '1-month': maersk_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(maersk_data, time_interval)
...
plt.title("GBM Simulations for Maersk (MAERSK-B.CO)")
