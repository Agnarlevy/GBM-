...
marel_data = yf.download("MAREL.AS", start="2023-01-01",
...
actual_price = marel_data['Close'].iloc[-1]

s0_points = {
    '1-day': marel_data['Close'].iloc[-2],
    '1-week': marel_data['Close'].iloc[-6],
    '1-month': marel_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(marel_data, time_interval)
...
plt.title("GBM Simulations for Marel (MAREL.AS)")
