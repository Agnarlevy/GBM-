...
novozymes_data = yf.download(
    "NZYM.VI", start="2023-01-01", end="2023-12-31", progress=False)
...
actual_price = novozymes_data['Close'].iloc[-1]

s0_points = {
    '1-day': novozymes_data['Close'].iloc[-2],
    '1-week': novozymes_data['Close'].iloc[-6],
    '1-month': novozymes_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(novozymes_data, time_interval)
...
plt.title("GBM Simulations for Novozymes (NZYM.VI)")
