...
unitedhealth_data = yf.download(
    "UNH", start="2023-01-01", end="2023-12-31", progress=False)
...
actual_price = unitedhealth_data['Close'].iloc[-1]


s0_points = {
    '1-day': unitedhealth_data['Close'].iloc[-2],
    '1-week': unitedhealth_data['Close'].iloc[-6],

    '1-month': unitedhealth_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(unitedhealth_data, time_interval)
...
plt.title("GBM Simulations for UnitedHealth Group (UNH)")
