...
walmart_data = yf.download("WMT", start="2023-01-01",
...
actual_price = walmart_data['Close'].iloc[-1]

s0_points = {
    '1-day': walmart_data['Close'].iloc[-2],
    '1-week': walmart_data['Close'].iloc[-6],
    '1-month': walmart_data['Close'].iloc[-21]
}
...
for horizon, time_interval in time_horizons.items():
    ...
    drift, sigma_star = calculate_parameters(walmart_data, time_interval)
...
plt.title("GBM Simulations for Walmart (WMT)")
